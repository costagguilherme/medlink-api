...
import med.voll.api.dtos.medicos.MedicoDto;
import med.voll.api.dtos.medicos.MedicoGetResponseDto;
import med.voll.api.dtos.medicos.MedicoPutDto;
...
public class MedicosController extends BaseController {
    ...
    @PutMapping("/{id}")
    @Transactional
    public ResponseEntity<Medico> update(@RequestBody @Valid MedicoPutDto data, @PathVariable Long id) {
        Medico medico = this.medicoRepository.findById(id).orElse(null);
        if (medico == null) {
            return (ResponseEntity<Medico>) this.errorResponse(HttpStatus.NOT_FOUND, "Médico não encontrado");
        }
